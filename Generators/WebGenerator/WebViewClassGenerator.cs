...
namespace voidsoft
{
	public partial class WebClassGenerator
	{
		...
		private  string GenerateWebViewMarkup(EntityData e)
		{
			...
			EntityProperty linkProperty = GetLinkField(e, properties);

			if (linkProperty != null)
			{
				b.Append(Environment.NewLine);
				b.Append("<asp:TemplateField ItemStyle-HorizontalAlign='Left' HeaderText='" + linkProperty.PropertyName + "'>");
				b.Append(Environment.NewLine);
				b.Append("<ItemTemplate>");
				b.Append(Environment.NewLine);
				b.Append("<asp:LinkButton CommandArgument='<%#DataBinder.Eval(Container.DataItem, " + Constants.QUOTE + primaryKeyFieldName + Constants.QUOTE + ")%>' CommandName='linkSelect' runat='server' Text='<%#DataBinder.Eval(Container.DataItem, " + Constants.QUOTE + linkProperty.PropertyName + Constants.QUOTE + ")%>'></asp:LinkButton>");
				b.Append(Environment.NewLine);
				b.Append("</ItemTemplate>");
				b.Append(Environment.NewLine);
				b.Append("</asp:TemplateField>");
			...
			foreach (var p in properties)
			{
				...
				if (p.PropertyName != primaryKeyFieldName)
				{
					if (linkProperty != null)
					{
						if (p.PropertyName == linkProperty.PropertyName)
						{
							continue;
						}
					}
		...
		private EntityProperty GetLinkField(EntityData e, EntityProperty[] fields)
		{
			...
			foreach (EntityProperty property in fields)
			{
				//also look for 
		...
		private static string GetControlReadValueProperty(EntityProperty e)
		{
